...
基于 **Intel Gaudi2 8卡**的多模态大模型训练底座工程，专注于**暖通/机电工程图构件识别**任务。

**应用场景**：输入工程图纸图像 + 文字 → 输出 think 推理 + 识别结果（风管三通、弯头、变径等构件类别）
...
- **🧠 Think 推理范式**
  - 训练可解释 AI，让模型学会推理
  - 输出格式：`<think>推理过程</think>\n这是{构件类别}`

- **🔧 暖通构件识别**
  - 支持8种暖通构件：风管三通、弯头、变径、阀门等
  - 批量数据生成工具，从CSV标注表快速构建数据集
  - 完整评估和推理工具链
